...
$backendCmd = "cd '$projectRoot'; uv run uvicorn federated_pneumonia_detection.src.api.main:app --reload --host 127.0.0.1 --port 8001"
Start-Process powershell -ArgumentList "-NoExit", "-Command", $backendCmd

# Start frontend in new terminal
Write-Host "[Frontend] Starting Vite dev server on http://localhost:5173" -ForegroundColor Green
$frontendCmd = "cd '$projectRoot\xray-vision-ai-forge'; npm run dev"
