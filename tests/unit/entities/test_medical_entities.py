...
class TestCustomImageDataset:
    ...
    @pytest.fixture
    def valid_df(self):
        return pd.DataFrame({"filename": ["sample.png"], "Target": [1]})
    ...
    def test_init_success(self, temp_data_structure, valid_df):
        ...
        SampleDataFactory.create_dummy_image().save(
            Path(temp_data_structure["base_path"]) / "sample.png",
        )
    ...
    def test_get_class_distribution(self, temp_data_structure):
        ...
        df = pd.DataFrame({"filename": ["a.png", "b.png"], "Target": [0, 1]})
        base_path = Path(temp_data_structure["base_path"])
    ...
    def test_get_memory_usage_estimate_all_sample_fail(
        self,
        temp_data_structure,
        valid_df,
    ):
...
class TestResNetWithCustomHead:
    ...
    def test_init_invalid_params(self):
        ...
        with pytest.raises(
            ValueError,
            match="dropout_rate must be between 0.0 and 1.0",
        ):
            ResNetWithCustomHead(dropout_rate=1.5)

        with pytest.raises(
            ValueError,
            match="fine_tune_layers_count must be an integer",
        ):
    ...
    def test_custom_head_sizes_invalid(self):
        ...
        with pytest.raises(
            ValueError,
            match="custom_head_sizes must have at least 2 elements",
        ):
    ...
    def test_backbone_creation_failure(self):
        ...
        with patch(
            ...
            with pytest.raises(
                RuntimeError,
                match="Failed to create ResNet50 backbone",
            ):
                ResNetWithCustomHead()
