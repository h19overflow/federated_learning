...
class TestCentralizedTrainer:
    ...
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.ConfigManager")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.DataSourceExtractor")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.os.makedirs")
    ...
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.ConfigManager")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.get_session")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.run_crud")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.load_metadata")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.create_train_val_split")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.XRayDataModule")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.LitResNetEnhanced")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.create_trainer_from_config")
    @patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.TensorBoardLogger")
    def test_train_workflow(self, mock_tb_logger, mock_create_trainer, mock_model_class, 
                            mock_datamodule_class, mock_split, mock_load, mock_run_crud, 
                            mock_get_session, mock_config_class, mock_config):
        ...
        mock_split.return_value = (pd.DataFrame({"Target": [0]}), pd.DataFrame({"Target": [1]}))
        ...
        trainer.data_source_extractor.extract_and_validate.return_value = ("img_dir", "csv_path")
        ...
        mock_trainer.fit.assert_called_once_with(mock_model, mock_datamodule_class.return_value)
        
        # Verify database interaction
        mock_run_crud.create.assert_called_once()
        mock_run_crud.complete_run.assert_called_once_with(mock_db, run_id=1, status="completed")
    ...
    def test_create_data_module(self, mock_config_class, mock_config):
        ...
        with patch("federated_pneumonia_detection.src.control.dl_model.centralized_trainer.XRayDataModule") as mock_dm:
