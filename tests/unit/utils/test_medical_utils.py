...
class TestDataProcessingFunctions:
    ...
    def test_load_metadata_no_config_none(self, temp_data_structure):
        ...
        with patch(
            ...
            df = load_metadata(
                temp_data_structure["metadata_path"],
                config_or_constants=None,
            )
    ...
    def test_load_metadata_deprecated_constants(self, temp_data_structure):
        ...
        with pytest.warns(
            DeprecationWarning,
            match="Passing SystemConstants to load_metadata is deprecated",
        ):
    ...
    def test_sample_dataframe_invalid_fraction(self):
        ...
        with pytest.raises(
            ValueError,
            match="sample_fraction must be between 0.0 and 1.0",
        ):
    ...
    def test_create_train_val_split_success(self):
        ...
        df = pd.DataFrame({"Target": [0, 0, 0, 0, 1, 1, 1, 1]})
    ...
    def test_create_train_val_split_invalid_split(self):
        ...
        with pytest.raises(
            ValueError,
            match="validation_split must be between 0.0 and 1.0",
        ):
    ...
    def test_load_and_split_data_deprecated_constants(
        self,
        temp_data_structure_custom,
        sample_config,
    ):
    ...
    def test_get_data_statistics(self):
        ...
        df = pd.DataFrame({"Target": [0, 0, 1, None]})
...
class TestImageTransforms:
    ...
    def test_build_training_transforms(self, sample_config):
        ...
        t3 = builder.build_training_transforms(
            custom_preprocessing={"contrast_stretch": True},
        )
    ...
    def test_create_custom_transform(self, sample_config):
        ...
        t1 = builder.create_custom_transform(
            resize_strategy="random_crop",
            augmentations=["rotation", "horizontal_flip"],
        )
        t2 = builder.create_custom_transform(
            resize_strategy="pad_resize",
...
def test_logger_setup():
    ...
    from federated_pneumonia_detection.src.internals.loggers.logger import (
        get_logger,
        setup_logger,
    )
