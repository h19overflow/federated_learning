...
async def update_settings(
    ...
    # Synchronize federated configuration to pyproject.toml (lazy import)
    try:
        ...
        # flwr not installed - skip TOML sync (federated features unavailable)
        logger.debug("Skipping TOML sync - flwr not installed")
    except Exception as e:
        # Log the error but don't fail the entire update
        # The YAML config was successfully updated even if TOML sync fails
        logger.warning(f"TOML sync failed: {e}")
