...
class TextReportExporter(DataExporter):
    ...
    def export(self, data: Dict[str, Any]) -> str:
        ...
        lines.append(f"Status: {data.get('status', 'N/A')} | Epochs: {metadata.get('total_epochs', 'N/A')}")
        final = data.get("final_metrics", {})
        lines += ["", "FINAL METRICS", "-" * 80]
        for k, v in [("Acc", "accuracy"), ("Recall", "recall"), ("F1", "f1_score"), ("AUC", "auc")]:
            val = final.get(v, 0)
        ...
        if history:
            lines += ["", "HISTORY", "-" * 80]
            for e in history[:5]:
                lines.append(f"E{e.get('epoch', 1):<2} | TL:{e.get('train_loss', 0):.3f} "
                           f"VL:{e.get('val_loss', 0):.3f} VA:{e.get('val_acc', 0)*100:.1f}%")
        lines += ["", "=" * 80, f"Report: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "=" * 80]
        return "\n".join(lines)
