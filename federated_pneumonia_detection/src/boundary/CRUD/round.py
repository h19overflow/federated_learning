...
class RoundCRUD:
    ...
    def start_round(self, round_id: int):
        ...
        with get_session() as session:
            round_instance = session.query(Round).filter(Round.id == round_id).first()
            if round_instance:
                round_instance.start_time = datetime.now(timezone.utc)
                session.flush()
                session.commit()
                return True
            return False
    ...
    def complete_round(self, round_id: int):
        ...
        with get_session() as session:
            round_instance = session.query(Round).filter(Round.id == round_id).first()
            if round_instance:
                round_instance.end_time = datetime.now(timezone.utc)
                session.flush()
                session.commit()
                return True
            return False
    ...
    def get_or_create_round(self, db_session, client_id: int, round_number: int) -> int:
        ...
        with get_session() as session:
            try:
                new_round = Round(client_id=client_id, round_number=round_number)
                session.add(new_round)
                session.commit()
                return new_round
