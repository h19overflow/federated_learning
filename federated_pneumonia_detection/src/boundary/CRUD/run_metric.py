...
class RunMetricCRUD(BaseCRUD[RunMetric]):
    ...
    def get_by_metric_name(self, db: Session, run_id: int, metric_name: str) -> List[RunMetric]:
        """Get specific metric for a run."""
        return db.query(self.model).filter(
            self.model.run_id == run_id,
            self.model.metric_name == metric_name
        ).order_by(self.model.step).all()
    
    def get_by_dataset_type(self, db: Session, run_id: int, dataset_type: str) -> List[RunMetric]:
    ...
    def get_latest_by_metric(self, db: Session, run_id: int, metric_name: str) -> Optional[RunMetric]:
        """Get the latest value for a specific metric."""
        return db.query(self.model).filter(
            self.model.run_id == run_id,
            self.model.metric_name == metric_name
        ).order_by(self.model.step.desc()).first()
