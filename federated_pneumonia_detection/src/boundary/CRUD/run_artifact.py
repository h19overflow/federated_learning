...
class RunArtifactCRUD(BaseCRUD[RunArtifact]):
    ...
    def get_by_type(self, db: Session, run_id: int, artifact_type: str) -> List[RunArtifact]:
    ...
    def get_by_name(self, db: Session, run_id: int, artifact_name: str) -> Optional[RunArtifact]:
        """Get a specific artifact by name."""
        return db.query(self.model).filter(
            self.model.run_id == run_id,
            self.model.artifact_name == artifact_name
        ).first()
    ...
    def search_by_name(self, db: Session, run_id: int, search_term: str) -> List[RunArtifact]:
