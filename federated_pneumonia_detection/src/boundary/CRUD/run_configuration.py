...
class RunConfigurationCRUD(BaseCRUD[RunConfiguration]):
    ...
    def get_by_partition_strategy(
        self, db: Session, strategy: str
    ) -> List[RunConfiguration]:
        """Get configurations by partition strategy."""
        return (
            db.query(self.model).filter(self.model.partition_strategy == strategy).all()
        )

    def get_by_learning_rate(
        self, db: Session, learning_rate: float
    ) -> List[RunConfiguration]:
        """Get configurations by learning rate."""
        return (
            db.query(self.model).filter(self.model.learning_rate == learning_rate).all()
        )
