...
class QueryEngine:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise e
        try:
            self.vectorstore = PGVector(connection=Settings().get_postgres_db_uri(), collection_name='research_papers',embeddings=self.embeddings)
        except Exception as e:
            logging.getLogger(__name__).error(f"Error initializing the vectorstore: {e}")
            raise e
