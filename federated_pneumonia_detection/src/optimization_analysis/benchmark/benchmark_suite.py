    ...
    def run_benchmark(
        ...
        for i, img in enumerate(images):
            try:
                ...
                with timer.time_stage("total"):
                    # This is already accounted for by individual stages
                    pass
        ...
        for i in range(
            ...
            preprocess_time = (
                ...
                if "data" in stage_stats["preprocessing"]
                else 0
            )
            feature_time = (
                stage_stats["feature_extraction"]["data"][i]
                if "data" in stage_stats["feature_extraction"]
                else 0
            )
            classify_time = (
                stage_stats["classification"]["data"][i]
                if "data" in stage_stats["classification"]
                else 0
            )
...
def create_default_suite(image_dir: str, num_samples: int = 1000) -> BenchmarkSuite:
    ...
    suite = BenchmarkSuite(image_dir=image_dir, num_samples=num_samples)
    return suite
