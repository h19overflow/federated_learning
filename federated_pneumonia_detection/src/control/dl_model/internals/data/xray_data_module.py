...
class XRayDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.test_df = (
            test_df.copy() if test_df is not None and not test_df.empty else None
        )
        self.image_dir = Path(image_dir) if image_dir else Path(".")

        self.color_mode = (
            color_mode or self.config.get("experiment.color_mode", "RGB")
        ).upper()
        num_workers = self.config.get("experiment.num_workers", 4)
        self.validate_images_on_init = (
            validate_images_on_init
            if validate_images_on_init is not None
            else self.config.get("experiment.validate_images_on_init", True)
        )
        ...
        self.pin_memory = (
            ...
            else self.config.get("experiment.pin_memory", True)
        ) and torch.cuda.is_available()
        self.persistent_workers = (
            persistent_workers
            if persistent_workers is not None
            else self.config.get("experiment.persistent_workers", False)
        ) and num_workers > 0
        self.prefetch_factor = (
            prefetch_factor
            if prefetch_factor is not None
            else (
                self.config.get("experiment.prefetch_factor", 2)
                if num_workers > 0
                else 2
            )
        )
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        train_transforms = create_training_transforms(
            ...
            self.custom_preprocessing_config
            if self.custom_preprocessing_config
            else None,
        )
