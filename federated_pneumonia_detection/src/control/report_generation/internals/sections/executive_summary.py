...
def _create_metric_box(label: str, value: str, color) -> Table:
    ...
    return Table(
        [[Paragraph(
            f"<b>{value}</b>",
            ParagraphStyle('MetricValue', fontSize=18, textColor=color, alignment=TA_CENTER)
        )],
         [Paragraph(
             label,
             ParagraphStyle('MetricLabel', fontSize=8, textColor=colors.gray, alignment=TA_CENTER)
         )]],
        colWidths=[1.4*inch]
    )
