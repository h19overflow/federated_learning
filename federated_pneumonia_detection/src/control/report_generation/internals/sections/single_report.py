...
def build_single_report(
    ...
    _add_probability_breakdown(story, styles, pneumonia_probability, normal_probability)
...
def _add_probability_breakdown(
    ...
    prob_data = [
        ...
        [
            "PNEUMONIA",
            f"{pneumonia_probability * 100:.1f}%",
        ...
        if orig_rl:
            images_row.append(
                [
                    ...
                    Paragraph(
                        "Original X-Ray",
                        ParagraphStyle("ImgCaption", fontSize=9, alignment=TA_CENTER),
                    ),
                ],
            )
...
def _add_clinical_interpretation(
    ...
    if clinical_interpretation.get("summary"):
        ...
        story.append(
            Paragraph(clinical_interpretation["summary"], styles["ReportBody"]),
        )
        story.append(Spacer(1, 8))
    ...
    if clinical_interpretation.get("confidence_explanation"):
        ...
        story.append(
            Paragraph(
                clinical_interpretation["confidence_explanation"],
                styles["ReportBody"],
            ),
        )
        story.append(Spacer(1, 8))
    ...
    if risk:
        ...
        risk_style = (
            ...
            else ("RiskModerate" if risk_level == "MODERATE" else "RiskLow")
        )
...
def _add_disclaimer(story: list, styles) -> None:
    ...
    story.append(
        Paragraph(
            "<b>IMPORTANT DISCLAIMER:</b> This report is generated by an "
            "AI-assisted diagnostic system and is intended for informational "
            "purposes only. It should NOT be used as the sole basis for clinical "
            "decisions. The predictions and interpretations provided must be "
            "reviewed and validated by a qualified radiologist or physician. "
            "AI systems can make errors, and clinical judgment should always take "
...
def _add_footer(story: list, model_version: str) -> None:
    ...
    story.append(
        Paragraph(
            f"Model: {model_version} | Report ID: {report_id} | "
