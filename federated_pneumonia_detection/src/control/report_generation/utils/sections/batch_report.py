...
def build_batch_report(
    ...
    create_header(
        story, styles, "Batch Analysis Report",
...
def _add_methodology(story: list, styles, model_version: str) -> None:
    ...
    story.append(Paragraph(
        ...
        "(Normal vs Pneumonia). The model uses GradCAM (Gradient-weighted Class Activation "
...
def _add_disclaimer(story: list, styles) -> None:
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle(
            'DisclaimerTitle', fontSize=10, textColor=DANGER_COLOR, fontName='Helvetica-Bold'
        )
    ))
    story.append(Paragraph(
...
def _add_footer(story: list, model_version: str) -> None:
    ...
    footer_data = [[
        Paragraph("XRay Vision AI", ParagraphStyle(
            'FootL', fontSize=7, textColor=colors.gray
        )),
        Paragraph(f"Model: {model_version}", ParagraphStyle(
            'FootC', fontSize=7, textColor=colors.gray, alignment=TA_CENTER
        )),
        Paragraph(f"Report ID: {datetime.now().strftime('%Y%m%d%H%M%S')}", ParagraphStyle(
