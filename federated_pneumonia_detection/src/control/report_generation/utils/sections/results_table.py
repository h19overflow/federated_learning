...
def create_results_table(story: list, styles, results: list) -> None:
    ...
    results_header = [
        ...
        Paragraph("<b>Filename</b>", ParagraphStyle(
            'TH', fontSize=8, textColor=colors.white
        )),
        Paragraph("<b>Classification</b>", ParagraphStyle(
            'TH', fontSize=8, textColor=colors.white, alignment=TA_CENTER
        )),
        Paragraph("<b>Confidence</b>", ParagraphStyle(
            'TH', fontSize=8, textColor=colors.white, alignment=TA_CENTER
        )),
        Paragraph("<b>Pneumonia %</b>", ParagraphStyle(
            'TH', fontSize=8, textColor=colors.white, alignment=TA_CENTER
        )),
        Paragraph("<b>Normal %</b>", ParagraphStyle(
            'TH', fontSize=8, textColor=colors.white, alignment=TA_CENTER
        )),
    ]
    ...
    results_table = Table(
        results_data,
        colWidths=[0.35*inch, 2.1*inch, 1.1*inch, 0.85*inch, 0.9*inch, 0.85*inch]
    )
...
def _build_result_row(index: int, result: dict) -> list:
    ...
    return [
        ...
        Paragraph(
            f"{pneumonia_prob:.1f}%", ParagraphStyle('TD', fontSize=8, alignment=TA_CENTER)
        ),
        Paragraph(
