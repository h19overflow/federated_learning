...
class XRayPreprocessor:
    ...
    def contrast_stretch_percentile(
        ...
        p_lower, p_upper = np.percentile(img_array, (lower_percentile, upper_percentile))

        # Apply contrast stretching
        img_array = np.clip(
            (img_array - p_lower) / (p_upper - p_lower + eps),
            0, 1
        )
        ...
        **kwargs
        ...
        def preprocess_fn(image: Image.Image) -> Image.Image:
            try:
                ...
                self.logger.warning(f"Preprocessing failed, returning original image: {e}")
                return image
...
class TransformBuilder:
    ...
    def build_test_time_augmentation_transforms(
        ...
        for i in range(num_augmentations):
            ...
            rotation_angle = (i - num_augmentations // 2) * 5  # -10, -5, 0, 5, 10 degrees
    ...
    def create_custom_transform(
        ...
        if resize_strategy == 'resize_crop':
            ...
            transform_list.append(transforms.Resize(img_size, transforms.InterpolationMode.BILINEAR))
        ...
        if preprocessing:
            preprocess_fn = self.preprocessor.create_custom_preprocessing_pipeline(**preprocessing)
            transform_list.append(transforms.Lambda(preprocess_fn))
    ...
    **kwargs
    ...
    if use_custom_preprocessing:
        preprocessing_config = {
            ...
            **{k: v for k, v in kwargs.items() if k.startswith(('lower_', 'upper_', 'clip_', 'edge_'))}
        }
