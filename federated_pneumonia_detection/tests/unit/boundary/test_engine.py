...
from sqlalchemy.types import String as StringType, Text as TextType, Integer as IntegerType, Float as FloatType, TIMESTAMP as TimestampType
...
class TestExperimentModel:
    """Tests for Experiment SQLAlchemy model."""

    def test_experiment_table_name(self):
        """Test that Experiment model has correct table name."""
        assert Experiment.__tablename__ == 'experiments'

    def test_experiment_columns_exist(self):
        """Test that Experiment has required columns."""
        required_columns = {'id', 'name', 'description', 'created_at'}
        actual_columns = {col.name for col in Experiment.__table__.columns}
        assert required_columns.issubset(actual_columns)

    def test_experiment_id_is_primary_key(self):
        """Test that id column is primary key."""
        id_column = Experiment.__table__.columns['id']
        assert id_column.primary_key is True

    def test_experiment_created_at_default(self):
        """Test that created_at has default value."""
        created_at_column = Experiment.__table__.columns['created_at']
        assert created_at_column.default is not None

    def test_experiment_runs_relationship(self):
        """Test that Experiment has runs relationship."""
        assert hasattr(Experiment, 'runs')
        # Verify relationship exists in the mapper
        assert 'runs' in Experiment.__mapper__.relationships

    def test_experiment_name_string_length(self):
        """Test that name column is String type with length."""
        name_column = Experiment.__table__.columns['name']
        assert isinstance(name_column.type, StringType)

    def test_experiment_description_text_type(self):
        """Test that description column is Text type."""
        description_column = Experiment.__table__.columns['description']
        assert isinstance(description_column.type, TextType)

    def test_experiment_created_at_timestamp_type(self):
        """Test that created_at column is TIMESTAMP type."""
        created_at_column = Experiment.__table__.columns['created_at']
        assert isinstance(created_at_column.type, TimestampType)
...
class TestRunModel:
    ...
    def test_run_required_columns(self):
        ...
        required_columns = {
            'id', 'experiment_id', 'training_mode', 'status',
            'start_time', 'end_time', 'wandb_id', 'source_path'
        }
    ...
    def test_run_experiment_id_is_foreign_key(self):
        """Test that experiment_id is foreign key."""
        experiment_id_column = Run.__table__.columns['experiment_id']
        assert len(experiment_id_column.foreign_keys) > 0
        # Check it references experiments table
        assert any('experiments' in str(fk) for fk in experiment_id_column.foreign_keys)

    def test_run_experiment_relationship(self):
        """Test that Run has experiment relationship."""
        assert hasattr(Run, 'experiment')
        assert 'experiment' in Run.__mapper__.relationships
...
class TestRunConfigurationModel:
    ...
    def test_run_configuration_required_columns(self):
        ...
        required_columns = {
            'id', 'run_id', 'learning_rate', 'epochs', 'weight_decay',
            'batch_size', 'num_rounds', 'num_clients', 'clients_per_round',
            'local_epochs', 'partition_strategy', 'seed'
        }
    ...
    def test_run_configuration_partition_strategy_string(self):
        ...
        partition_strategy_column = RunConfiguration.__table__.columns['partition_strategy']
...
class TestRunArtifactModel:
    ...
    def test_run_artifact_required_columns(self):
        ...
        required_columns = {
            'id', 'run_id', 'artifact_name', 'artifact_path', 'artifact_type'
        }
...
class TestModelRelationships:
    ...
    def test_experiment_run_bidirectional_relationship(self):
        """Test bidirectional relationship between Experiment and Run."""
        assert hasattr(Experiment, 'runs')
        assert hasattr(Run, 'experiment')
...
class TestBaseMetadata:
    ...
    def test_all_models_use_base(self):
        """Test that all models inherit from Base."""
        models = [Experiment, Run, RunConfiguration, RunMetric, RunArtifact]
        for model in models:
    ...
    def test_base_registry_contains_all_tables(self):
        ...
        expected_tables = {
            'experiments', 'runs', 'run_configurations',
...
class TestCreateTablesFunction:
    ...
    def test_create_tables_calls_create_engine(self, mock_settings, mock_create_engine):
        ...
        mock_settings.get_postgres_db_uri.return_value = 'postgresql://user:pass@localhost/db'
    ...
    @patch('federated_pneumonia_detection.src.boundary.engine.create_engine')
    @patch('federated_pneumonia_detection.src.boundary.engine.settings')
    def test_create_tables_calls_metadata_create_all(self, mock_settings, mock_create_engine):
...
class TestGetSessionFunction:
    ...
    @patch('federated_pneumonia_detection.src.boundary.engine.create_engine')
    @patch('federated_pneumonia_detection.src.boundary.engine.settings')
    def test_get_session_creates_engine(self, mock_settings, mock_create_engine, mock_sessionmaker):
    ...
    @patch('federated_pneumonia_detection.src.boundary.engine.create_engine')
    @patch('federated_pneumonia_detection.src.boundary.engine.settings')
    def test_get_session_creates_session_factory(self, mock_settings, mock_create_engine, mock_sessionmaker):
    ...
    @patch('federated_pneumonia_detection.src.boundary.engine.create_engine')
    @patch('federated_pneumonia_detection.src.boundary.engine.settings')
    def test_get_session_returns_session(self, mock_settings, mock_create_engine, mock_sessionmaker):
    ...
    @patch('federated_pneumonia_detection.src.boundary.engine.create_engine')
    @patch('federated_pneumonia_detection.src.boundary.engine.settings')
    def test_get_session_uses_postgres_uri(self, mock_settings, mock_create_engine, mock_sessionmaker):
    ...
    @patch('federated_pneumonia_detection.src.boundary.engine.create_engine')
    @patch('federated_pneumonia_detection.src.boundary.engine.settings')
    def test_get_session_multiple_calls_create_new_sessions(self, mock_settings, mock_create_engine, mock_sessionmaker):
...
class TestColumnConstraints:
    ...
    def test_experiment_name_not_nullable(self):
        """Test that experiment name column properties."""
        name_column = Experiment.__table__.columns['name']
        # Column exists and has proper type
...
class TestForeignKeyConstraints:
    ...
    def test_run_references_experiments(self):
        """Test that Run.experiment_id references Experiment.id."""
        run_experiment_fk = Run.__table__.columns['experiment_id'].foreign_keys
        assert len(run_experiment_fk) > 0
        assert any('experiments.id' in str(fk) for fk in run_experiment_fk)
