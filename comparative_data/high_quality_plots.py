"""
High-Quality Visualization Script for FYP2
===========================================
Generates publication-quality plots for Chapter 6: Results & Analysis

Key Improvements:
- DPI 300+ for print quality
- Vector format (PDF + PNG) support
- Improved font sizes and line weights
- Better anti-aliasing
- Optimized for LaTeX documents
"""
...
plt.rcParams.update({
    # Figure settings
    'figure.figsize': (10, 6),
    'figure.dpi': 300,  # High DPI for screen preview
...
# Save formats
SAVE_FORMATS = ['png', 'pdf']  # Both raster and vector
...
def save_figure(fig, filename_base):
    ...
    for fmt in SAVE_FORMATS:
        filepath = PLOTS_DIR / f"{filename_base}.{fmt}"
        fig.savefig(filepath, format=fmt, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"  Created: {filename_base} ({', '.join(SAVE_FORMATS)})")
...
def plot_metric_comparison(stats):
    ...
    for i, metric in enumerate(metrics):
        if stats['statistical_tests'][metric]['paired_t_test']['significant']:
            ...
            ax.annotate('*', xy=(i, max_h + 0.04), ha='center', fontsize=20,
                       color='red', fontweight='bold')

    plt.tight_layout()
    save_figure(fig, "fig_6_1_metric_comparison")
...
def plot_recall_boxplot(stats, cent_df, fed_df):
    ...
    bp = ax.boxplot(data, positions=positions, widths=0.5, patch_artist=True,
                    showfliers=False, medianprops=dict(color='black', linewidth=3),
    ...
    for i, (d, seeds) in enumerate(zip(data, [SEEDS, SEEDS])):
        ...
        scatter = ax.scatter(x, d, alpha=0.8, color='black', s=100, zorder=3,
                           edgecolors='white', linewidth=2)
        for xi, yi, seed in zip(x, d, seeds):
            ax.annotate(f'{seed}', (xi + 0.08, yi), fontsize=10, alpha=0.8,
                       fontweight='bold')

    ax.set_xticklabels(['Centralized\n(n=5)', 'Federated\n(n=5)'], fontsize=14)
    ax.set_ylabel('Recall (Sensitivity)', fontsize=14, fontweight='bold')
    ax.set_title('Recall Distribution by Training Paradigm', fontsize=16,
    ...
    ax.axhline(y=cent_mean, xmin=0.1, xmax=0.4, color=CENT_COLOR,
              linestyle='--', linewidth=2.5, alpha=0.8)
    ax.axhline(y=fed_mean, xmin=0.6, xmax=0.9, color=FED_COLOR,
              linestyle='--', linewidth=2.5, alpha=0.8)
    ...
    # Draw bracket
    ax.plot([1, 1, 2, 2], [0.99, 1.0, 1.0, 0.99], 'k-', linewidth=2)
    ax.annotate(f'{sig_text}\nCohen\'s d = {cohens_d:.2f}', xy=(1.5, 1.005),
               ha='center', fontsize=12, fontweight='bold')

    # Add mean annotations
    ax.annotate(f'μ = {cent_mean:.3f}', xy=(0.7, cent_mean), fontsize=11,
               color=CENT_COLOR, fontweight='bold')
    ax.annotate(f'μ = {fed_mean:.3f}', xy=(2.15, fed_mean), fontsize=11,
               color=FED_COLOR, fontweight='bold')

    plt.tight_layout()
    save_figure(fig, "fig_6_2_recall_boxplot")
...
def plot_baseline_comparison(stats, baseline):
    ...
    bars2 = ax.bar(x, cent_vals, width, label='Tuned Centralized',
                   color=CENT_COLOR, alpha=0.85, edgecolor='black', linewidth=1.5)
    bars3 = ax.bar(x + width, fed_vals, width, label='Tuned Federated',
                   color=FED_COLOR, alpha=0.85, edgecolor='black', linewidth=1.5)

    ax.set_ylabel('Score', fontsize=14, fontweight='bold')
    ax.set_title('Model Performance: Baseline vs Tuned Models', fontsize=16,
                 fontweight='bold', pad=15)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontsize=13)
    ax.legend(loc='lower right', fontsize=13, framealpha=0.95)
    ...
    plt.tight_layout()
    save_figure(fig, "fig_6_3_baseline_comparison")
...
def plot_cv_comparison(stats):
    ...
    # Add threshold lines
    ax.axhline(y=10, color='red', linestyle='--', alpha=0.6, linewidth=2)
    ax.axhline(y=5, color='orange', linestyle='--', alpha=0.6, linewidth=2)
    ax.annotate('High variability (10%)', xy=(4.5, 10.5), fontsize=11,
               color='red', fontweight='bold')
    ax.annotate('Moderate (5%)', xy=(4.5, 5.5), fontsize=11,
    ...
    plt.tight_layout()
    save_figure(fig, "fig_6_4_cv_comparison")
...
def plot_statistical_tests(stats):
    ...
    bars1 = ax1.barh(labels, p_values, color=colors, alpha=0.8,
                     edgecolor='black', linewidth=1.5)
    ax1.axvline(x=0.05, color='red', linestyle='--', linewidth=2.5, label='α = 0.05')
    ax1.set_xlabel('p-value', fontsize=14, fontweight='bold')
    ax1.set_title('Statistical Significance (Paired t-test)', fontsize=15, fontweight='bold')
    ...
    bars2 = ax2.barh(labels, cohens_d, color=effect_colors, alpha=0.8,
                     edgecolor='black', linewidth=1.5)
    ax2.axvline(x=0.8, color='green', linestyle='--', alpha=0.7, linewidth=2)
    ax2.axvline(x=0.5, color='orange', linestyle='--', alpha=0.7, linewidth=2)
    ax2.axvline(x=0.2, color='red', linestyle='--', alpha=0.7, linewidth=2)
    ax2.set_xlabel("|Cohen's d|", fontsize=14, fontweight='bold')
    ax2.set_title("Effect Size (Cohen's d)", fontsize=15, fontweight='bold')
    ...
    plt.tight_layout()
    save_figure(fig, "fig_6_5_statistical_tests")
...
def plot_confusion_matrices(stats):
    ...
    for idx, paradigm in enumerate(['centralized', 'federated']):
        ...
        sns.heatmap(cm_norm, annot=False, cmap='Blues' if paradigm == 'federated' else 'Greens',
                    ...
                    yticklabels=['Actual\nNormal', 'Actual\nPneumonia'],
                    cbar_kws={'label': 'Percentage (%)'}, linewidths=2, linecolor='white')
        ...
        axes[idx].set_title(f'{paradigm.title()} Confusion Matrix{title_extra}',
                           fontsize=15, fontweight='bold', pad=10)

    plt.suptitle('Aggregate Confusion Matrices (Seeds 44-48, 5 Runs)',
                fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
    save_figure(fig, "fig_6_6_confusion_matrices")
...
def plot_fnr_fpr_tradeoff(stats):
    ...
    ax.scatter(fprs, fnrs, s=250, c=FED_COLOR, alpha=0.8, edgecolors='black',
              linewidth=2.5, label='Federated Runs', zorder=5)

    for fpr, fnr, seed in zip(fprs, fnrs, seeds):
        ax.annotate(f'Seed {seed}', (fpr + 0.015, fnr + 0.003), fontsize=11,
                   fontweight='bold')

    # Plot centralized aggregate point
    cent_agg = stats['fairness_metrics']['centralized']['aggregate']
    ax.scatter([cent_agg['fpr']], [cent_agg['fnr']], s=300, c=CENT_COLOR, marker='s',
              alpha=0.8, edgecolors='black', linewidth=2.5, label='Centralized (Approx)', zorder=5)
    ax.annotate('Centralized', (cent_agg['fpr'] + 0.015, cent_agg['fnr'] + 0.003),
               fontsize=11, fontweight='bold')

    # Risk zones
    ax.axhline(y=0.15, color='red', linestyle=':', alpha=0.6, linewidth=2)
    ax.axhline(y=0.05, color='orange', linestyle=':', alpha=0.6, linewidth=2)
    ax.axvline(x=0.20, color='red', linestyle=':', alpha=0.6, linewidth=2)

    # Fill zones
    ax.fill_between([0, 0.20], [0, 0], [0.05, 0.05], alpha=0.15, color='green',
                    label='Low Risk Zone')
    ax.fill_between([0, 0.20], [0.05, 0.05], [0.15, 0.15], alpha=0.15, color='yellow',
                    label='Moderate Risk')
    ax.fill_between([0, 1], [0.15, 0.15], [1, 1], alpha=0.15, color='red',
                    label='High Risk Zone')

    # Labels
    ax.set_xlabel('False Positive Rate (FPR) - Unnecessary Treatment',
                 fontsize=14, fontweight='bold')
    ax.set_ylabel('False Negative Rate (FNR) - Missed Pneumonia',
                 fontsize=14, fontweight='bold')
    ...
    ax.annotate('← Clinical Priority:\nMinimize FNR', xy=(0.02, 0.18), fontsize=12,
               color='darkred', style='italic', fontweight='bold')

    plt.tight_layout()
    save_figure(fig, "fig_6_7_fnr_fpr_tradeoff")
...
def plot_radar_comparison(stats):
    ...
    ax.plot(angles, cent_vals, 'o-', linewidth=3, color=CENT_COLOR,
           label='Centralized', markersize=10)
    ax.fill(angles, cent_vals, alpha=0.25, color=CENT_COLOR)
    ax.plot(angles, fed_vals, 'o-', linewidth=3, color=FED_COLOR,
           label='Federated', markersize=10)
    ...
    ax.set_title('Overall Performance Profile Comparison\n(Seeds 44-48)',
                size=16, y=1.08, fontweight='bold')
    ...
    for angle, cent_val, fed_val in zip(angles[:-1], cent_vals[:-1], fed_vals[:-1]):
        ax.annotate(f'{cent_val:.3f}', xy=(angle, cent_val + 0.05),
                   fontsize=10, color=CENT_COLOR, ha='center', fontweight='bold')
        ax.annotate(f'{fed_val:.3f}', xy=(angle, fed_val - 0.08),
                   fontsize=10, color=FED_COLOR, ha='center', fontweight='bold')

    plt.tight_layout()
    save_figure(fig, "fig_6_8_radar_comparison")
...
def plot_fairness_heatmap(stats):
    ...
    sns.heatmap(df, annot=True, fmt='.3f', cmap='RdYlGn', ax=ax, center=0.5,
                linewidths=1, cbar_kws={'label': 'Score'}, vmin=0, vmax=1,
                annot_kws={'fontsize': 12, 'fontweight': 'bold'})

    ax.set_title('Per-Run Fairness Metrics (Federated)\nGreen = Better, Red = Worse',
                fontsize=16, fontweight='bold', pad=15)
    ax.set_xlabel('Metric', fontsize=14, fontweight='bold')
    ax.set_ylabel('Run', fontsize=14, fontweight='bold')

    # Highlight FNR column (critical metric)
    for i in range(len(fed_runs)):
        ax.add_patch(plt.Rectangle((2, i), 1, 1, fill=False, edgecolor='black', linewidth=3))

    ax.annotate('← Critical Metric', xy=(3.1, 2.5), fontsize=12,
               color='darkred', fontweight='bold')

    plt.tight_layout()
    save_figure(fig, "fig_6_9_fairness_heatmap")
...
def plot_paired_differences(stats, cent_df, fed_df):
    ...
    for idx, (metric, label) in enumerate(zip(metrics, labels)):
        ...
        for i, seed in enumerate(SEEDS):
            ax.plot([0, 1], [cent_vals[i], fed_vals[i]], 'o-', color='gray',
                   alpha=0.5, linewidth=2)
            ax.scatter([0], [cent_vals[i]], color=CENT_COLOR, s=100, zorder=5,
                      edgecolors='black', linewidth=1.5)
            ax.scatter([1], [fed_vals[i]], color=FED_COLOR, s=100, zorder=5,
                      edgecolors='black', linewidth=1.5)
            ax.annotate(f'{seed}', (0.02, cent_vals[i]), fontsize=9, alpha=0.8,
                       fontweight='bold')

        # Add mean lines
        ax.axhline(y=cent_vals.mean(), xmin=0.05, xmax=0.35, color=CENT_COLOR,
                  linestyle='--', linewidth=2.5)
        ax.axhline(y=fed_vals.mean(), xmin=0.65, xmax=0.95, color=FED_COLOR,
                  linestyle='--', linewidth=2.5)

        ax.set_xticks([0, 1])
        ax.set_xticklabels(['Centralized', 'Federated'], fontsize=12)
        ax.set_ylabel(label, fontsize=13, fontweight='bold')
        ax.set_title(f'{label}\n(Δ = {diff.mean():+.4f})', fontsize=14, fontweight='bold')

        # Add significance marker
        p_val = stats['statistical_tests'][metric]['paired_t_test']['p_value']
        if p_val < 0.05:
            ax.annotate('*', xy=(0.5, ax.get_ylim()[1]), fontsize=20, color='red',
    ...
    # Add legend
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=CENT_COLOR,
                  markersize=12, label='Centralized'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=FED_COLOR,
                  markersize=12, label='Federated'),
        plt.Line2D([0], [0], color='gray', alpha=0.5, linewidth=2, label='Paired connection')
    ]
    axes[5].legend(handles=legend_elements, loc='center', fontsize=14)
    axes[5].set_title('Legend', fontsize=14, fontweight='bold')

    plt.suptitle('Paired Comparison by Seed (Lines connect same seed)',
                fontsize=16, fontweight='bold', y=1.00)
    plt.tight_layout()
    save_figure(fig, "fig_6_10_paired_differences")
...
def generate_all_plots():
    """Generate all high-quality plots for Chapter 6."""
    print("=" * 70)
    print("Generating HIGH-QUALITY Plots for Chapter 6 (Publication Ready)")
    print("=" * 70)
