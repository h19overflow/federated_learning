...
def load_data():
    ...
    baseline_payload = baseline_raw.get('evaluate_metrics_serverapp', {}).get('2')
    if isinstance(baseline_payload, dict):
        baseline = baseline_payload
    elif isinstance(baseline_payload, str):
        baseline = ast.literal_eval(baseline_payload)
    else:
        raise ValueError(
            "Unexpected baseline format: expected dict or string at "
            "baseline_raw['evaluate_metrics_serverapp']['2']"
        )
...
def approximate_confusion_matrix(recall, precision, total_positive, total_negative):
    ...
    """
    # Clamp rates into [0, 1] to avoid nonsense from upstream data issues.
    recall = float(np.clip(recall, 0.0, 1.0))
    precision = float(np.clip(precision, 0.0, 1.0))

    # Enforce integer counts that exactly sum back to the known class totals.
    tp = int(round(recall * total_positive))
    tp = int(np.clip(tp, 0, total_positive))
    fn = int(total_positive - tp)

    if precision > 0:
        fp = int(round(tp * (1.0 - precision) / precision))
    else:
        fp = int(total_negative)  # Worst case: everything predicted positive

    fp = int(np.clip(fp, 0, total_negative))
