            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={cn(
                  ...
                  {overallStatus === 'completed' && <CheckCircle2 className="h-6 w-6 text-[hsl(172_63%_25%)]" />}
                  {overallStatus === 'error' && <XCircle className="h-6 w-6 text-[hsl(0_72%_51%)]" />}
                </div>
                <div>
                  ...
                  <p className="text-[hsl(215_15%_50%)] mt-1">
                    ...
                    {overallStatus === 'completed' && 'Training completed successfully'}
                    {overallStatus === 'error' && 'Training encountered an error'}
                  </p>
                </div>
              </div>
              {getStatusBadge()}
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="bg-[hsl(168_25%_98%)] rounded-2xl p-6 border border-[hsl(168_20%_92%)]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-xl border border-[hsl(210_15%_94%)]">
                  <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide mb-1">Training Mode</p>
                  <p className="font-semibold text-[hsl(172_43%_20%)]">
                    {config.trainingMode === 'centralized' ? 'Centralized' : 'Federated Learning'}
                  </p>
                </div>
                <div className="bg-white p-4 rounded-xl border border-[hsl(210_15%_94%)]">
                  <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide mb-1">Learning Rate</p>
                  <p className="font-semibold font-mono text-[hsl(172_43%_20%)]">{config.learningRate}</p>
                </div>
                <div className="bg-white p-4 rounded-xl border border-[hsl(210_15%_94%)]">
                  <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide mb-1">
                    {config.trainingMode === 'centralized' ? 'Epochs' : 'Local Epochs'}
                  </p>
                  <p className="font-semibold text-[hsl(172_43%_20%)]">
                    {config.trainingMode === 'centralized' ? config.epochs : config.localEpochs}
                  </p>
                </div>
                <div className="bg-white p-4 rounded-xl border border-[hsl(210_15%_94%)]">
                  <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide mb-1">Batch Size</p>
                  <p className="font-semibold text-[hsl(172_43%_20%)]">{config.batchSize}</p>
                </div>
                {config.trainingMode === 'federated' && (
                  <>
                    <div className="bg-white p-4 rounded-xl border border-[hsl(210_15%_94%)]">
                      <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide mb-1">Clients</p>
                      <p className="font-semibold text-[hsl(172_43%_20%)]">{config.clients || 'Not set'}</p>
                    </div>
                    <div className="bg-white p-4 rounded-xl border border-[hsl(210_15%_94%)]">
                      <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide mb-1">Federated Rounds</p>
                      <p className="font-semibold text-[hsl(172_43%_20%)]">{config.federatedRounds || 'Not set'}</p>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="bg-[hsl(172_30%_12%)] rounded-2xl p-6 border border-[hsl(172_40%_20%)]">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-sm font-medium text-[hsl(168_30%_70%)]">Training Log</h4>
              </div>
              <div
                ref={logContainerRef}
              ...
              >
                {statusMessages.length === 0 ? (
                  <div className="text-[hsl(168_20%_50%)] italic py-4">
                    Ready to start experiment. Press the "Start Training" button below.
                  </div>
                ) : (
                  statusMessages.map((msg, idx) => (
                    <div
                      ...
                      className={cn(
                        ...
                        msg.type === 'error' && "text-[hsl(0_70%_65%)] font-semibold",
