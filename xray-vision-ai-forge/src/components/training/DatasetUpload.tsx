...
interface DatasetUploadProps {
  onComplete: (data: { file: File | null; trainSplit: number }) => void;
  initialData?: {
    file: File | null;
    trainSplit: number;
  };
...
const DatasetUpload = ({ onComplete, initialData }: DatasetUploadProps) => {
  ...
  const validateDataset = async (file: File) => {
    ...
    const steps: ValidationStep[] = [
      ...
      {
        id: "matching",
        name: "Matching Patient IDs with images",
        status: "pending",
      },
    ];
    ...
    try {
      ...
      for (const path of allFiles) {
        ...
        const imagesFolderIndex = parts.findIndex(
          (p) => p.toLowerCase() === "images",
        );
      ...
      if (!imagesFolder) {
        ...
        throw new Error(
          'Images directory not found. Please ensure your ZIP contains a folder named "Images"',
        );
      }
      ...
      const csvFiles = allFiles.filter(
        (path) => path.toLowerCase().endsWith(".csv") && !zip.files[path].dir,
      );
      ...
      if (csvFiles.length === 0) {
        ...
        throw new Error(
          "CSV file not found. Please include a metadata CSV file in your ZIP",
        );
      }
      ...
      if (patientIdIndex === -1) {
        ...
        throw new Error(
          `CSV must contain "patientId" column. Found: ${headers.join(", ")}`,
        );
      }
      ...
      if (targetIndex === -1) {
        updateValidationStep("csvFormat", {
          ...
          details: [
            ...
            "Note: Target column is recommended but not required",
          ],
        });
      } else {
        updateValidationStep("csvFormat", {
      ...
      if (unmatchedPatientIds.length > 0) {
        const percentage = (
          (unmatchedPatientIds.length / csvPatientIds.length) *
          100
        ).toFixed(1);
        warnings.push(
          `${unmatchedPatientIds.length} patient IDs (${percentage}%) don't have matching images`,
        );
      }

      if (imageFiles.length > csvPatientIds.length) {
        warnings.push(
          `${imageFiles.length - csvPatientIds.length} images without CSV entries`,
        );
      }
      ...
      if (matchedImages.length === 0) {
        ...
        if (ratio > 1.5) {
          warnings.push(
            `Patient IDs don't match image filenames. You may have multiple images per patient.`,
          );
          warnings.push(
            `Found ${imageFiles.length} images for ${csvPatientIds.length} patient records (${ratio.toFixed(1)}x ratio)`,
          );
          ...
          throw new Error(
            "Patient IDs in CSV do not match any image files. Check console for details.",
          );
        }
      }

      const matchPercentage = (
        (matchedImages.length / csvPatientIds.length) *
        100
      ).toFixed(1);
      ...
      updateValidationStep("matching", {
        status:
          unmatchedPatientIds.length > csvPatientIds.length * 0.1
            ? "warning"
            : "success",
  ...
  const isRestoredExperiment =
    initialData && initialData.trainSplit && !initialData.file && !file;
      ...
      >
        <div className="space-y-3">
          <p className="font-medium text-[hsl(172_43%_20%)]">
            Your dataset ZIP file must contain:
          </p>
          <ul className="space-y-2 ml-1">
            <li className="flex items-start gap-2">
              ...
              <span>
                ...
                A directory named "Images" containing chest X-ray images
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-[hsl(172_63%_28%)] mt-2 flex-shrink-0" />
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  <code className="px-2 py-1 rounded-lg bg-[hsl(168_25%_94%)] text-[hsl(172_43%_20%)] text-xs font-medium">
                    patientId
                  </code>
                  <span className="text-[hsl(215_15%_50%)] text-xs">
                    required
                  </span>
                  <code className="px-2 py-1 rounded-lg bg-[hsl(168_25%_94%)] text-[hsl(172_43%_20%)] text-xs font-medium">
                    Target / Label
          ...
          <p className="text-xs text-[hsl(215_15%_50%)] pt-2 border-t border-[hsl(168_20%_90%)]">
            <strong>Supported format:</strong> .zip files only |{" "}
            <strong>Maximum recommended:</strong> 5GB
          </p>
        </div>
      </InstructionCard>
          ...
          <div className="px-8 py-6 border-b border-[hsl(210_15%_94%)] bg-gradient-to-r from-[hsl(168_25%_98%)] to-white">
            <div className="flex items-center justify-between">
              ...
              <HelpTooltip
                ...
                content={
                  <div className="space-y-3">
                    <p className="text-[hsl(172_43%_20%)]">
...
└── metadata.csv`}
          ...
          <div className="p-8 space-y-8">
            ...
            >
              ...
              {isRestoredExperiment && (
                <div className="space-y-4">
                  ...
                  <p className="text-xl font-semibold text-[hsl(172_43%_15%)]">
                    Upload a dataset
                  </p>
                  <p className="text-[hsl(215_15%_50%)]">
                    No dataset recorded in current session
                  </p>
              ...
              {!isRestoredExperiment && uploadStatus === "idle" && (
                <div className="space-y-4">
                  ...
                  <div>
                    <p className="text-xl font-semibold text-[hsl(172_43%_15%)]">
                      Drop your dataset ZIP file here
                    </p>
                    <p className="text-[hsl(215_15%_50%)] mt-2">
                      ZIP file should contain chest X-ray images and a metadata
                      CSV
                    </p>
                  </div>
                  <Button
                    variant="outline"
                    className="mt-2 rounded-xl border-[hsl(172_30%_80%)] text-[hsl(172_43%_25%)] hover:bg-[hsl(168_25%_94%)]"
                  >
                    Select File
                  </Button>
                </div>
              ...
              {uploadStatus === "uploading" && (
                <div className="space-y-4">
                  ...
                  <div className="w-full bg-[hsl(168_25%_92%)] rounded-full h-2 max-w-xs mx-auto overflow-hidden">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-[hsl(172_63%_35%)] to-[hsl(172_63%_28%)] animate-pulse"
                      style={{ width: "60%" }}
                    />
                  </div>
                </div>
            ...
            {validationSteps.length > 0 && (
              <div
                className="bg-[hsl(168_25%_98%)] border border-[hsl(168_20%_92%)] rounded-2xl p-6"
                    ...
                    >
                      <Loader
                        className={cn(
                          "h-5 w-5",
                          uploading
                            ? "animate-spin text-[hsl(172_63%_35%)]"
                            : "text-[hsl(215_15%_55%)]",
                        )}
                      />
                    </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                      ...
                      <div className="bg-white/60 backdrop-blur p-4 rounded-xl border border-[hsl(168_20%_92%)]">
                        ...
                        <p className="text-2xl font-semibold text-[hsl(172_63%_28%)] mt-1">
                          {validationResult.matchedImages}
                        </p>
                      </div>
                      <div className="bg-white/60 backdrop-blur p-4 rounded-xl border border-[hsl(168_20%_92%)]">
                        <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide">
                        ...
                        <p className="text-2xl font-semibold text-[hsl(172_43%_20%)] mt-1">
                          {validationResult.targetValues.length > 0
                            ? validationResult.targetValues.length
                            : "N/A"}
                        </p>
                      </div>
                    </div>
                    ...
                    {validationResult.warnings.length > 0 && (
                      <div className="mb-4 p-4 rounded-xl bg-[hsl(35_60%_96%)] border border-[hsl(35_50%_85%)]">
                        <div className="flex items-center gap-2 mb-2">
                          <AlertTriangle className="h-4 w-4 text-[hsl(35_70%_45%)]" />
                          <p className="text-sm font-medium text-[hsl(35_70%_35%)]">
                            Warnings
                          </p>
                        </div>
                    ...
                    {validationResult.errors.length > 0 && (
                      <div className="mb-4 p-4 rounded-xl bg-[hsl(0_60%_97%)] border border-[hsl(0_50%_85%)]">
                        <div className="flex items-center gap-2 mb-2">
                          <X className="h-4 w-4 text-[hsl(0_72%_51%)]" />
                          <p className="text-sm font-medium text-[hsl(0_72%_40%)]">
                            Errors
                          </p>
                        </div>
                    ...
                    {validationResult.unmatchedPatientIds.length > 0 && (
                      <details className="mt-4">
                        <summary className="text-sm font-medium text-[hsl(215_15%_45%)] cursor-pointer hover:text-[hsl(172_43%_25%)] transition-colors">
                          View unmatched Patient IDs (
                          {validationResult.unmatchedPatientIds.length})
                        </summary>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-xs text-[hsl(215_15%_55%)] space-y-1">
                          <p>
                            ...
                            {validationResult.imagesFolder}
                          </p>
                          <p>
                            <strong>CSV file:</strong>{" "}
                            {validationResult.csvFile}
                          </p>
                          {validationResult.targetColumnName && (
                            <p>
                              <strong>
                                {validationResult.targetColumnName} values:
                              </strong>{" "}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white p-5 rounded-xl border border-[hsl(210_15%_92%)] shadow-sm">
                    ...
                    <p className="text-3xl font-semibold text-[hsl(172_43%_20%)] mt-1">
                      {datasetSummary?.totalImages || 5000}
                    </p>
                  </div>
                  <div className="bg-white p-5 rounded-xl border border-[hsl(210_15%_92%)] shadow-sm">
                    <p className="text-xs text-[hsl(215_15%_55%)] uppercase tracking-wide">
                    ...
                    <p className="text-3xl font-semibold text-[hsl(172_43%_20%)] mt-1">
                      ...
                      datasetSummary.classes.length > 0
                  ...
                  content={
                    <div className="space-y-2">
                      <p>
                        ...
                        Used to train the model's weights
                      </p>
                      <p>
                        <strong className="text-[hsl(172_43%_20%)]">
                          Validation Set:
              ...
              <InstructionCard variant="tip" className="animate-fade-in">
                <p>
                  ...
                  {datasetSummary && datasetSummary.totalImages < 500 && (
                    <>
                      {" "}
                      Since your dataset is smaller, consider using 70/30 for
