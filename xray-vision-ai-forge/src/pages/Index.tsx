...
const Index = () => {
  ...
  const [experimentConfig, setExperimentConfig] = useState<ExperimentConfiguration>({
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.viewRunId) {
      ...
      const loadFederatedState = async () => {
        try {
          ...
          if (fedData.is_federated && fedData.rounds && fedData.rounds.length > 0) {
            ...
            console.log('[Index] ⚠️ No federated data found or run is not federated:', {
      ...
      // Go to results step
      const targetStep = (location.state.goToStep && typeof location.state.goToStep === 'number')
    ...
    if (location.state && location.state.loadedExperiment) {
      ...
      if (location.state.goToStep && typeof location.state.goToStep === 'number') {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return <DatasetUpload onComplete={handleDatasetComplete} initialData={datasetState} />;
      case 2:
      ...
      case 4:
        ...
        return runId ? (
          ...
          <div className="text-center p-8">
            <p className="text-muted-foreground">No run ID available. Please start training first.</p>
            <Button onClick={handleReset} className="mt-4">Start New Experiment</Button>
          </div>
        );
      default:
        return <DatasetUpload onComplete={handleDatasetComplete} initialData={datasetState} />;
